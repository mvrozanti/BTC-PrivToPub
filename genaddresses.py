...
def main():
    ...
    if private_key is not None:
        ...
        leMoneyShot['private'] = "{}".format(base58_check(private_key + bytes([0x01]), version_bytes=coin['private_prefix']))
    ...
    if public_key is not None:
        ...
        leMoneyShot['public'] = "{}".format(addr)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        keypair = main()
        count+=1
        walletBalance = json.loads(request.urlopen("https://blockchain.info/rawaddr/{}".format(keypair['public'])).read().decode('utf-8'))['final_balance']/100000000
        if walletBalance > 0:
            import winsound
            while True:
                ...
                print('\n')
                print('\n')
        keypair['balance'] = walletBalance
        print(keypair.values())
        end = time.time()
        totalTime += end - start
        averageTime = count / totalTime
        print('Average creation time:{}/s'.format(averageTime))